...
#'
#' @export
fuzzy.GK.Imp<-function(X,...) UseUseMethod("fuzzy.GK.Imp")
fuzzy.GK.Imp.default <- function(X,K=2,m=1.5,max.iteration=1000,
                         threshold=10^-9,RandomNumber=0,rho=rep(1,K),
                         gamma=0) {
  library(MASS)
  data.X <- as.matrix(X)
  n <- nrow(data.X)
  ...
  class(result)<-"fuzzy.gk.imp"
  result
  return(result)
}

print.fuzzy.gk.imp<-function(x,..){
  cat("Call:\n")
  print(x$call)
